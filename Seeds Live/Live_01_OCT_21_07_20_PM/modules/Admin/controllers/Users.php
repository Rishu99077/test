...
class Users extends MX_Controller {
	...
	private function validateLogin(){
		...
		if($userrole!='1' AND $userrole!='2' AND $userrole!='3' AND $userrole!='5' AND $userrole!='6' AND $userrole!='7'){
			redirect('admin/logout');
			exit();  
		}
		if($user_permission!=''){
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST')){
			...
			if ($this->form_validation->run() == FALSE) {
		        ...
		        if ($this->input->post('userpermission',true)!=''){
	            	$userpermission = json_encode($this->input->post('userpermission',true));
	            }else{
	            	$userpermission = '';
	            }
				...
				$data['userrole'] = $userprofile_data->userrole;
				$data['userstatus'] = $userprofile_data->userstatus;
	...
	public function deleteuser_old(){
		$validateLogin = $this->validateLogin();
		$data['userpermission']=$validateLogin['userpermission'];
		$data['userrole']=$validateLogin['userrole'];
		...
		if($UserID AND $UserID!='1'){
			...
			$delete_userUserID = $this->user->delete_user($UserID);
			if($delete_userUserID){
				$datalog = array();
				$datalog['UserID'] = $this->session->userdata('UserID');
				$datalog['Activity'] = 'Delete';
				$datalog['Title'] = 'User Deleted '.$userprofiledata->firstname." ".$userprofiledata->lastname;
