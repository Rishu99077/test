        ...
        <div class="page-header">
            ...
            <div class="page-header-breadcrumb">
                <ul class="breadcrumb-title">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.inclusion') }}">{{ $common['title'] }}</a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('admin.inclusion.add') }}"
                                class="btn btn-primary waves-effect waves-light f-right d-inline-block md-trigger"
                                data-modal="modal-13"> <i
                                    class="icofont icofont-plus m-r-5"></i>{{ translate('Add Inclusions') }}
                            </a>
                        </div>
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($Inclusion) > 0)
                                        @foreach ($Inclusion as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_inclusion->firstItem() }}</th>

                                                <td><img class="tbl-img-css"
                                                        src="{{ url('uploads/inclusions', $value['image']) }}"></td>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.inclusion.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.inclusion.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_inclusion->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
