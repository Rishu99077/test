                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('admin.productsfood.tags.add') }}"
                                class="btn btn-primary waves-effect waves-light f-right d-inline-block md-trigger"
                                data-modal="modal-13"> <i class="icofont icofont-plus m-r-5"></i>{{ translate('Add Tags') }}
                            </a>
                        </div>
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($Products_tags) > 0)
                                        @foreach ($Products_tags as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_products_tags->firstItem() }}</th>
                                                <td>{{ $value['title'] }}</td>
                                                <td>{{ $value['type'] == 'food_tags' ? 'Food Tag' : 'Drink Tag' }}</td>
                                                <td>
                                                    {!! checkStatus($value['status']) !!}
                                                </td>
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.productsfood.tags.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.productsfood.tags.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_products_tags->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
