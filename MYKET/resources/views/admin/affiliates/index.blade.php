                        ...
                        <div class="card p-20 pb-0 pt-1">
                            <div class="row">
                                ...
                                <div class="col-sm-3 form-group">
                                    <label class="col-form-label"> {{ translate('Affiliate Name') }}</label>
                                    <div>
                                        <input type="text" class="form-control" placeholder="Affiliate Name"
                                            name="affiliate_name" value="{{ $common['affiliate_name'] }}">
                                    </div>
                                </div>

                                <div class="col-sm-2 form-group">
                                    <label class="col-form-label"> {{ translate('Affiliate Code') }}</label>
                                    <div>
                                        <input type="text" class="form-control" placeholder="Affiliate Code"
                                            name="affiliate_code" value="{{ $common['affiliate_code'] }}">
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-2 form-group">
                                    ...
                                    <div>
                                        <select name="affiliate_status" class="form-control">
                                            <option value=""></option>
                                            <option value="Active" {{ getSelected($common['affiliate_status'], 'Active') }}>
                                                {{ translate('Active') }}
                                            </option>
                                            <option value="Deactive"
                                                {{ getSelected($common['affiliate_status'], 'Deactive') }}>
                                                {{ translate('Deactive') }}
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-sm-2 form-group">
                                    <label class="form-label" for="affiliate_date">{{ translate('Date') }}</label>
                                    <input class="form-control datetimepicker" id="timepicker3" name="affiliate_date"
                                        type="text" placeholder="Y-m-d to Y-m-d" value="{{ $common['affiliate_date'] }}"
                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h5>
                                {{ translate('Total') . ' ' . $common['title'] . ' (' . $get_affiliates_count . ')' }}
                            </h5>
                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($get_affiliates) > 0)
                                        @foreach ($get_affiliates as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_affiliates->firstItem() }}</th>

                                                <td>
                                                    <img class="tbl-img-css"
                                                        src="{{ $value['image'] != '' ? url('uploads/user_image', $value['image']) : asset('uploads/placeholder/placeholder.png') }}">
                                                </td>
                                                ...
                                                <td><?php echo $value['affiliate_code']; ?></td>
                                                <td>
                                                    {!! checkStatus($value['status']) !!}
                                                </td>
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.affiliates.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.affiliates.delete', encrypt($value['id'])) }}"
                                                            class="btn btn-danger btn-icon text-white confirm-delete">
                                                            <i class="icofont icofont-ui-delete"></i></a>
                                                        <a href="{{ route('admin.affiliates.affilliate_coupon', encrypt($value['id'])) }}"
                                                            class="btn btn-info btn-icon text-white">
                                                            <i class="icofont icofont-sale-discount "></i></a>
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_affiliates->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
