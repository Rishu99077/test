                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('admin.blogcategory.add') }}"
                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($Blogcategory) > 0)
                                        @foreach ($Blogcategory as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_blogcategory->firstItem() }}</th>

                                                <td><img class="custom_img"
                                                        src="{{ url('uploads/blogcategory', $value['image']) }}"></td>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.blogcategory.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.blogcategory.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_blogcategory->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
