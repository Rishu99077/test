                                ...
                                <div class="col-sm-4 form-group">
                                    ...
                                    <div>
                                        <input type="text" class="form-control" placeholder="{{ translate('Order ID') }}"
                                            name="hotel_order_id" value="{{ $common['ht_hotel_order_id'] }}">
                                    </div>
                                </div>

                                <div class="col-sm-4 form-group">
                                    <label class="form-label" for="hotel_date">{{ translate('Order Date') }}</label>
                                    <input class="form-control datetimepicker" id="timepicker3" name="hotel_date"
                                        type="text" placeholder="Y-m-d to Y-m-d" value="{{ $common['ht_hotel_date'] }}"
                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($get_hotel_commisions) > 0)
                                        @foreach ($get_hotel_commisions as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_hotel_commisions->firstItem() }}
                                                </th>
                                                <td>
                                                    <a target="_blank"
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        ...
                                                        <a href="{{ route('admin.hotel_commission_view', encrypt($value['ht_com_id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_hotel_commisions->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
