        ...
        <div class="page-header">
            ...
            <div class="page-header-breadcrumb">
                <ul class="breadcrumb-title">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.product_type') }}">{{ $common['title'] }}</a>
                    </li>
                </ul>
            </div>
        </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('admin.product_type.add') }}"
                                ...
                                data-modal="modal-13"> <i class="icofont icofont-plus m-r-5"></i> Add {{ $common['title'] }}
                            </a>
                        </div>
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($ProductType) > 0)
                                        @foreach ($ProductType as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_product_type->firstItem() }}</th>
                                                <td>{{ $value['title'] }}</td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.product_type.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.product_type.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_product_type->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
