                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h5>{{ translate('Transportation') }}</h5>
                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($Transportation) > 0)
                                        @foreach ($Transportation as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_transportation->firstItem() }}</th>
                                                <td>{{ $value['title'] }}</td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.transportation.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.transportation.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_transportation->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
