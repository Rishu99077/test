                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($Guide) > 0)
                                        @foreach ($Guide as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_guide->firstItem() }}</th>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.guide.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.guide.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_guide->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
