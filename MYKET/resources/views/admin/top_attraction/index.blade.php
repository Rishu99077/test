                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('admin.top_attraction.add') }}"
                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($TopAttraction) > 0)
                                        @foreach ($TopAttraction as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_top_attraction->firstItem() }}</th>
                                                <td>
                                                    <img class="custom_img"
                                                        src="{{ asset('uploads/top_attraction/' . $value['image']) }}"
                                                        alt="">
                                                </td>
                                                <td>{{ $value['title'] }}</td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.top_attraction.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.top_attraction.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_top_attraction->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
