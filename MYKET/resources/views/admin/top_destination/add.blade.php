        ...
        <div class="page-header">
            ...
            <div class="page-header-breadcrumb">
                <ul class="breadcrumb-title">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.top_destination') }}">{{ $common['title'] }}</a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-block">
                            <form id="main" method="post" action="{{ route('admin.top_destination.add') }}"
                                enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <input type="hidden" name="id" value="{{ $get_top_destination['id'] }}">
                                    <div class="col-md-6">
                                        <div class="form-group mb-3 {{ $errors->has('title') ? 'has-danger' : '' }}">
                                            ...
                                            <input
                                                ...
                                                value="{{ old('title', $get_top_destination_language['title']) }}"
                                    ...
                                    <div class="col-md-6">
                                        <div
                                            ...
                                            <label class="col-form-label">{{ translate('short_description') }}<span
                                                    class="mandatory cls" style="color:red; font-size:15px">*</span></label>
                                            <input
                                                class="form-control {{ $errors->has('short_description') ? 'form-control-danger' : '' }}"
                                                name="short_description" type="text"
                                                value="{{ old('short_description', $get_top_destination_language['short_description']) }}"
                                        ...
                                        <div class="media-left">
                                            <a href="#" class="profile-image">
                                                <img class="user-img img-circle img-css" id="image"
                                                    src="{{ $get_top_destination['image'] != '' ? url('uploads/top_destination', $get_top_destination['image']) : asset('uploads/placeholder/placeholder.png') }}">
                                            </a>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group mb-3 {{ $errors->has('country') ? 'has-danger' : '' }}">
                                            ...
                                            <select id="country" name="country" onchange="getStateCity('country')"
                                                ...
                                                @foreach ($Countries as $C)
                                                    <option value="{{ $C['id'] }}"
                                                        {{ getSelected($C['id'], $get_top_destination['country']) }}>
                                                        {{ $C['name'] }}</option>
                                                @endforeach
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group mb-3 {{ $errors->has('state') ? 'has-danger' : '' }}">
                                            ...
                                            <select id="state" name="state" onchange="getStateCity('state')"
                                                ...
                                                @foreach ($States as $S)
                                                    <option value="{{ $S['id'] }}"
                                                        {{ getSelected($S['id'], $get_top_destination['state']) }}>
                                                        {{ $S['name'] }}</option>
                                                @endforeach
                                            </select>
                                            @error('state')
                                        ...
                                        <div class="form-group mb-3 {{ $errors->has('city') ? 'has-danger' : '' }}">
                                            ...
                                            <select id="city" name="city" class="form-control select2">
                                                ...
                                                @foreach ($Cities as $CI)
                                                    <option value="{{ $CI['id'] }}"
                                                        {{ getSelected($CI['id'], $get_top_destination['city']) }}>
                                                        {{ $CI['name'] }}</option>
                                                @endforeach
                                            </select>
                                            @error('city')
                                        ...
                                        <div class="form-group mb-3 {{ $errors->has('status') ? 'has-danger' : '' }}">
                                            ...
                                            <select id="status" name="status" class="form-control stock">
                                                ...
                                                <option value="Deactive"
                                                    {{ getSelected($get_top_destination['status'], 'Deactive') }}>
                                                    {{ translate('Deactive') }}
                                                </option>
                                            </select>
                                            @error('status')
