                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ route('admin.top_destination.add') }}"
                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($TopDestination) > 0)
                                        @foreach ($TopDestination as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_top_destination->firstItem() }}</th>
                                                <td>
                                                    <img class=" custom_img"
                                                        src="{{ asset('uploads/top_destination/' . $value['image']) }}"
                                                        alt="">
                                                </td>
                                                <td>{{ $value['title'] }}</td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.top_destination.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.top_destination.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_top_destination->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
