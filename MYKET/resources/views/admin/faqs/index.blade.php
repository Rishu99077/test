        ...
        <div class="page-header">
            ...
            <div class="page-header-breadcrumb">
                <ul class="breadcrumb-title">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.faqs') }}">{{ $common['title'] }}</a>
                    </li>
                </ul>
            </div>
        </div>
                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($Faqs) > 0)
                                        @foreach ($Faqs as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_faqs->firstItem() }}</th>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.faqs.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.faqs.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_faqs->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
