                                ...
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($categories) > 0)
                                        @foreach ($categories as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_category->firstItem() }}</th>

                                                <td><img class="tbl-img-css"
                                                        src="{{ url('uploads/categories', $value['image']) }}"></td>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.categories.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.categories.delete', encrypt($value['id'])) }}"
