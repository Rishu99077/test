                        ...
                        <div class="card-block">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="example-2">
                                    ...
                                    @if (count($Blogs) > 0)
                                        @foreach ($Blogs as $key => $value)
                                            <tr>
                                                <th scope="row">{{ $key + $get_blogs->firstItem() }}</th>

                                                <td><img class="custom_img"
                                                        src="{{ url('uploads/blogs', $value['image']) }}"></td>
                                                ...
                                                <td>
                                                    <div class="action_cls mt-2">
                                                        <a href="{{ route('admin.blogs.edit', encrypt($value['id'])) }}"
                                                            class="btn btn-success btn-icon">
                                                            <i class="icofont icofont-ui-edit"></i></a>
                                                        <a data-href="{{ route('admin.blogs.delete', encrypt($value['id'])) }}"
                                ...
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {{ $get_blogs->appends(request()->query())->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
