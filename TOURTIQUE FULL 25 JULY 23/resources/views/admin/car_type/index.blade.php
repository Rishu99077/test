...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="col-auto ms-auto">
            <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.car_type.add') }}" type="button"><span
                        ...
                        <div class="tab-pane preview-tab-pane active" role="tabpanel"
                            ...
                            <div class="table-responsive scrollbar">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php $i = 1; ?>
                                        @if (!$get_car_type->isEmpty())
                                            @foreach ($get_car_type as $key => $value)
                                                <tr>
                                                    <td>{{ $key + $get_car_type->firstItem() }}</td>
                                                    <td>
                                                        @php
                                                            $get_car_type_language = App\Models\CarTypeLanguage::where('car_type_id', $value['id'])->get();
                                                        @endphp
                                                        @if($get_car_type_language)
                                                            {{  getLanguageTranslate($get_car_type_language, $lang_id, $value['id'], 'title', 'car_type_id')   }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-end">
                                                        <div>
                                                            <a class="btn p-0"
                                                                href="{{ route('admin.car_type.edit', encrypt($value['id'])) }}"
                                                            ...
                                                            <a class="btn p-0" onclick="return doconfirm();"
                                                                href="{{ route('admin.car_type.delete', encrypt($value['id'])) }}"
                                                                type="button" data-bs-toggle="tooltip"
                            ...
                            <div>
                                {{ $get_car_type->appends(request()->query())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
