...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <form action="" method="get" class="mb-3 search-form">
            <div class="row">
                <div class="col-lg-5">
                    <input type="text" name="Location" class="form-control search-input" value="{{@$_GET['Location']}}" placeholder="{{ translate('Location Name') }}">
                </div>
                <div class="col-lg-5">
                    <select class="form-select single-select" name="Zone"  >
                      <option value="">{{ translate('Select Zone') }}</option>
                      @foreach ($zones as $val)
                          <option value="{{ $val['id'] }}"
                          {{ getSelected($val['id'], old('Zone', @$_GET['Zone'])) }}
                          > {{ $val['zone_title'] }}</option>
                      @endforeach
                </select>
                </div>
        ...
        <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.locations.exportLocation',['location'=>request()->Location,'Zone'=>request()->Zone]) }}" type="button">
                <span class="fas fa-plus-circle text-primary "></span> {{ translate('Download') }}</a>
            <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.locations.add') }}" type="button">
                        ...
                        <div class="tab-pane preview-tab-pane active" role="tabpanel"
                            ...
                            <div class="table-responsive scrollbar">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php $i = 1; ?>
                                        @if (!$get_locations->isEmpty())
                                            @foreach ($get_locations as $key => $value)
                                                <tr>
                                                    <td>{{ $key + $get_locations->firstItem() }}</td>
                                                    ...
                                                    <td class="text-end">
                                                        <div>
                                                            <a class="btn p-0"
                                                                href="{{ route('admin.locations.edit', encrypt($value['id'])) }}"
                                                            ...
                                                            <a class="btn p-0" onclick="return doconfirm();"
                                                                href="{{ route('admin.locations.delete', encrypt($value['id'])) }}"
                                                                type="button" data-bs-toggle="tooltip"
                            ...
                            <div>
                                {{ $get_locations->appends(request()->query())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
