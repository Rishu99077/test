...
<div class="row faq_div">
    ...
    <div class="col-md-6 mb-3">
        ...
        <input class="form-control {{ $errors->has('facility_title.' . $lang_id) ? 'is-invalid' : '' }}"
            ...
            type="text" value="{{ getLanguageTranslate($get_giftcard_page_facilities_language, $lang_id, $GPF['id'], 'facility_title', 'gift_card_facilities_id') }}" />
        ...
        <textarea class="form-control  {{ $errors->has('facility_description.' . $lang_id) ? 'is-invalid' : '' }} footer_text" placeholder="{{ translate('Enter Facilities Description') }}" id="title" name="facility_description[{{ $lang_id }}][]"> {{ getLanguageTranslate($get_giftcard_page_facilities_language, $lang_id, $GPF['id'], 'facility_description', 'gift_card_facilities_id') }}
        </textarea>
        ...
        <div class="col-lg-3 mt-2">
            <div class="avatar shadow-sm img-thumbnail position-relative blog-image-cls">
                <div class="h-100 w-100  overflow-hidden position-relative">
                    <img src="{{ $GPF['facility_image'] != '' ? asset('uploads/GiftCardPage/' . $GPF['facility_image']) : asset('uploads/placeholder/placeholder.png') }}" id="upload_work_logo_<?php echo $data; ?>" width="100" alt="" />
                </div>
            </div>
        </div>
    </div>   
