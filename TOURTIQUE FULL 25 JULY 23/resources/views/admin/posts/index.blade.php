...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="col-auto ms-auto">
            <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.posts.add') }}" type="button"><span
                ...
                <div class="card-body pt-0">
                    <div class="tab-content">
                        <div class="tab-pane preview-tab-pane active" role="tabpanel"
                                    ...
                                    <tbody>
                                        ...
                                        @if (!$getPost->isEmpty())
                                            @foreach ($getPost as $key => $value)
                                                <tr>
                                                    <td>{{ $key + $getPost->firstItem() }}</td>
                                                    <td>
                                                        @php
                                                            $get_post_language = App\Models\PostLanguage::where('post_id', $value['id'])->get();
                                                        @endphp
                                                        @if($get_post_language)
                                                            {{  getLanguageTranslate($get_post_language, $lang_id, $value['id'], 'description', 'post_id')   }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-end">
                                                        <div>
                                                            <a class="btn p-0"
                                                                href="{{ route('admin.posts.edit', encrypt($value['id'])) }}"
                                                            ...
                                                            <a class="btn p-0" onclick="return doconfirm();"
                                                                href="{{ route('admin.posts.delete', encrypt($value['id'])) }}"
                                                                type="button" data-bs-toggle="tooltip"
                            ...
                            <div>
                                {{ $getPost->appends(request()->query())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
