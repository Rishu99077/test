...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="col-auto ms-auto">
            <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.bus_type.add') }}" type="button"><span
                        ...
                        <div class="tab-pane preview-tab-pane active" role="tabpanel"
                            ...
                            <div class="table-responsive scrollbar">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php $i = 1; ?>
                                        @if (!$get_bus_type->isEmpty())
                                            @foreach ($get_bus_type as $key => $value)
                                                <tr>
                                                    <td>{{ $key + $get_bus_type->firstItem() }}</td>
                                                    <td>
                                                        @php
                                                            $get_bus_type_language = App\Models\BusTypeLanguage::where('bus_type_id', $value['id'])->get();
                                                        @endphp
                                                        @if($get_bus_type_language)
                                                            {{  getLanguageTranslate($get_bus_type_language, $lang_id, $value['id'], 'title', 'bus_type_id')   }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-end">
                                                        <div>
                                                            <a class="btn p-0"
                                                                href="{{ route('admin.bus_type.edit', encrypt($value['id'])) }}"
                                                            ...
                                                            <a class="btn p-0" onclick="return doconfirm();"
                                                                href="{{ route('admin.bus_type.delete', encrypt($value['id'])) }}"
                                                                type="button" data-bs-toggle="tooltip"
                            ...
                            <div>
                                {{ $get_bus_type->appends(request()->query())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
