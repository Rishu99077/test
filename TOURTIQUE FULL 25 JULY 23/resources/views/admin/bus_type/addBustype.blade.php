...
@section('content')
    ...
    <form class="row g-3 " method="POST" action="{{ route('admin.bus_type.add') }}" enctype="multipart/form-data">
        @csrf
            ...
            <div class="card mb-3">
                ...
                <div class="card-body ">
                    <div class="row">
                        ...
                        <input id="" name="id" type="hidden" value="{{ $get_bus_type['id'] }}" />

                        <div class="col-md-12 content_title">
                            <div class="form-check form-switch pl-0">
                                <input class="form-check-input float-end status switch_button"
                                    {{ getChecked('Active', old('status', $get_bus_type['status'])) }} id="status"
                        ...
                        <div class="col-md-6">
                            <label class="form-label"
                                for="title">{{ translate('Bus type') }}<span
                                    class="text-danger">*</span>
                            </label>
                            <input class="form-control {{ $errors->has('title.' . $lang_id) ? 'is-invalid' : '' }}"
                                placeholder="{{ translate('Title') }}" id="title" name="title[{{ $lang_id }}]"
                                type="text"
                                value="{{ getLanguageTranslate($get_bus_type_language, $lang_id, $get_bus_type['id'], 'title', 'bus_type_id') }}" />
                            @error('title.' . $lang_id)
