...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="col-auto ms-auto">
            <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.car_details.add') }}" type="button"><span
                ...
                <div class="card-body pt-0">
                    <div class="tab-content">
                        <div class="tab-pane preview-tab-pane active" role="tabpanel"
                                    ...
                                    <tbody>
                                        ...
                                        @if (!$get_details->isEmpty())
                                            @foreach ($get_details as $key => $value)
                                                <tr>
                                                    <td>{{ $key + $get_details->firstItem() }}</td>
                                                    <td>
                                                        @php
                                                            $get_car_details_language = App\Models\CarDetailLanguage::where('car_details_id', $value['id'])->get();
                                                        @endphp
                                                        @if($get_car_details_language)
                                                            {{  getLanguageTranslate($get_car_details_language, $lang_id, $value['id'], 'title', 'car_details_id')   }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-end">
                                                        <div>
                                                            <a class="btn p-0"
                                                                href="{{ route('admin.car_details.edit', encrypt($value['id'])) }}"
                                                            ...
                                                            <a class="btn p-0" onclick="return doconfirm();"
                                                                href="{{ route('admin.car_details.delete', encrypt($value['id'])) }}"
                                                                type="button" data-bs-toggle="tooltip"
                            ...
                            <div>
                                {{ $get_details->appends(request()->query())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
