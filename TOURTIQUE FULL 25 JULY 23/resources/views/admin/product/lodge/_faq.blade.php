...
<div class="row faq_div">
    ...
    <div class="col-md-6 mb-3">
        <label class="form-label" for="title">{{ translate('Faq Title') }}
        </label>
        ...
        <label class="form-label" for="title">{{ translate('Faq Answer') }}
        </label>
        <textarea class="form-control  {{ $errors->has('answer.' . $lang_id) ? 'is-invalid' : '' }}"
            placeholder="{{ translate('Enter Faq Answer') }}" id="answer_{{ $lang_id }}_{{ $data }}" name="answer[{{ $lang_id }}][]">{{ getLanguageTranslate($get_product_faq_language, $lang_id, $GPF['id'], 'answer', 'faq_id') }}</textarea>
