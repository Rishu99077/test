...
<div class="row option_div" id="option_div{{ $data }}">
    ...
    <div class="col-md-12 content_title ">
        ...
        <h5 class="text-black mt-2 fw-semi-bold fs-0">
            <input class="form-check-input mt-2 private_tour_price " id="private_tour_price_{{ $data }}"
                type="checkbox" name="private_tour_price[{{ $data }}]"
                {{ $GPO['is_private_tour'] != 0 ? 'checked' : '' }} value="{{ $ProductPrivateTourPrice['id'] }}">
            <input type="hidden" name="private_tour_price_id[{{ $data }}]"
                value="{{ $ProductPrivateTourPrice['id'] }}">
            <label for="private_tour_price_{{ $data }}"
                class="text-black mt-2 fw-semi-bold fs-0">{{ translate('Private Tour Price') }}</label>
        </h5>
    </div>

    <div class="col-md-6">
        <label class="form-label" for="title">{{ translate('Tour Option') }}<span
                class="text-danger">*</span>
        </label>
        <input
            class="form-control index_tour_option Langauge_{{ $lang_id }} {{ $errors->has('tour_option.' . $lang_id) ? 'is-invalid' : '' }}"
            placeholder="{{ translate('Tour Option') }}" id="tour_option_{{ $lang_id }}_{{ $data }}"
    ...
    <div class="col-md-6">
        ...
        <textarea
            class="form-control index_addtional_info Langauge_{{ $lang_id }} {{ $errors->has('addtional_info.' . $lang_id) ? 'is-invalid' : '' }}"
            cols="50" row="3" placeholder="{{ translate('Enter Additional Info') }}"
            id="addtional_info_{{ $lang_id }}_{{ $data }}"
    ...
    {{-- --------------------Private Tour Price------------------------- --}}
    <div class="col-md-12 content_title ">
        <table class="private_tour_table table table-responsive  {{ $GPO['is_private_tour'] != 0 ? '' : 'd-none' }}">
            <thead>
                <tr>
                    <th>{{ translate('Price Per Car') }}</th>
                    <th>{{ translate('Total Allowed') }}</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>
                        <input type="text" class="form-control numberonly"
                            placeholder="{{ translate('Price Per Car') }}"
                            value="{{ $ProductPrivateTourPrice['private_per_price'] }}"
                            name="price_per_car[{{ $data }}]">
                    </td>
                    <td>
                        <input class="form-control numberonly " type="text"
                            name="total_allowed[{{ $data }}]"
                            placeholder="{{ translate('Total Allowed') }}"
                            value="{{ $ProductPrivateTourPrice['total_allowed'] }}">
                    </td>
                </tr>
            </tbody>
        </table>
    ...
    <div class="col-md-6 content_title tour_price_box {{ $GPO['is_private_tour'] != 0 ? 'd-none' : '' }}">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input class="form-check-input mt-2 tour_infant_allowed" id="flexCheckDefault"
                            id="option_status" type="checkbox" name="tour_infant_allowed[{{ $data }}]"
                            value="1"
                            @php if($GPO['product_id'] != "")
                            {
                                if($ProductTourPriceDetails['infant_allowed'] == 1)
                                {
                                    echo " checked";
                                }
                            }else{
                    ...
                    <td>
                        <input
                            class="form-control numberonly {{ $errors->has('tour_price_infant') ? 'is-invalid' : '' }}"
                            type="text" name="tour_price_infant[{{ $data }}]"
                            placeholder="{{ translate('Infant Price') }}"
                            @php if($ProductTourPriceDetails['infant_allowed'] != 1 && !isset($append)&& $get_product['id'] != "")
                            {
                                echo "value=' N/A' readonly";
                            }else{
                                echo "value=".$ProductTourPriceDetails['infant_price']."";
                            } @endphp>
                        @error('tour_price_infant')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        @enderror
    ...
    {{-- ------------------------------------- --}}
    <div class="col-md-6 content_title {{ $GPO['is_private_tour'] != 0 ? 'd-none' : '' }} maximum_minimum_box ">
        <h5 class="text-black mt-2 fw-semi-bold fs-0">{{ translate('Minimum & Maximum People') }}</h5>
        <div class="row mybox">
            <div class="col-md-6">
                <label class="form-label" for="title">{{ translate('Minimum People') }}
                </label>
                <input class="form-control numberonly" placeholder="{{ translate('Enter Minimum People') }}"
                    id="minimum_people[{{ $data }}]" name="minimum_people[{{ $data }}]"
                    type="text" value="{{ $GPO['minimum_people'] }}" />
            </div>

            <div class="col-md-6">
                <label class="form-label" for="title">{{ translate('Maximum People') }}
                </label>
                <input class="form-control numberonly" placeholder="{{ translate('Enter Maximum People') }}"
                    id="maximum_people[{{ $data }}]" name="maximum_people[{{ $data }}]"
                    type="text" value="{{ $GPO['maximum_people'] }}" />
            </div>
        </div>
    </div>
    {{-- ------------------------------------- --}}
    ...
    {{-- Tour Upgrade --}}
    <div>
        <button class="btn btn-sm btn-success mb-1 ml-20 w-25 add_tour_upgrade" type="button"
            data-val="{{ $data }}"> <span class="fa fa-plus"></span>
            {{ translate('Add Tour Upgrade') }}
        </button>
    </div>
    <div class="show_tour_upgrade{{ $data }} @if (empty($get_product_option_tour_upgrade)) {{ 'd-none' }} @endif">
        <table class="table table-responsive">
            <thead>
                <tr>
            ...
            <tbody class="show_tour_upgrade_tbody{{ $data }}">
                @php
                    $id = 1;
                    
                @endphp
                @foreach ($get_product_option_tour_upgrade as $KK => $GPOTU)
                    @if ($GPO['id'] == $GPOTU['product_option_id'])
                        @include('admin.product.excursion._tour_upgrade')
                    @else
                        @php
                            $id = 0;
                        @endphp
    ...
    {{-- Period Pricing --}}
    <div class="period_price_box {{ $GPO['is_private_tour'] != 0 ? 'd-none' : '' }}">
        <div>
            <button class="btn btn-sm btn-success mb-1 ml-20 w-25 add_period_pricing" type="button"
                data-val="{{ $data }}"> <span class="fa fa-plus"></span>
                {{ translate('Add Period Pricing') }}
            </button>
        </div>
        <div
            class="show_period_pricing{{ $data }}  @if (empty($get_product_option_period_pricing)) {{ 'd-none' }} @endif">
            <table class="table table-responsive">
                <thead>
                    <tr>
                ...
                <tbody class="show_period_pricing_tbody{{ $data }}">
                    @php
                        $id = 1;
                        
                    @endphp
                    @foreach ($get_product_option_period_pricing as $PP => $GPOPP)
                        @if ($GPO['id'] == $GPOPP['product_option_id'])
                            @include('admin.product.excursion._tour_period_pricing')
                        @else
                            @php
                                $id = 0;
                            @endphp
    ...
    <div class="col-md-6 content_title ">
        @php
            $ProductTaxSerivce = (array) getDataFromDB('product_option_tax_service', ['product_id' => $GPO['product_id'], 'product_option_id' => $GPO['id']], 'row');
        ...
        <table class="table table-responsive">
            <tbody>
                <tr>
                    <td> <input class="form-check-input mt-2 " id="option_tax_allowed_{{ $data }}"
                            type="checkbox" name="option_tax_allowed[{{ $data }}]" value="1"
                            @php if($GPO['product_id'] != "")
                            {
                        ...
                        <label for="option_tax_allowed_{{ $data }}"
    ...
    <div class="col-md-6 content_title">
        <table class="table table-responsive">
            <tbody>
                <tr>
                    <td> <input class="form-check-input mt-2 " id="tax_check_{{ $data }}" type="checkbox"
            ...
            <tbody class="show_tour_price_tbody{{ $data }}">
                ...
                @foreach ($get_product_option_week_tour as $KK => $GPOWT)
                    @if ($GPO['id'] == $GPOWT['product_option_id'])
                        @include('admin.product.excursion._tour_price')
                    @else
                        @php
                            $id = 0;
                        @endphp
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if ($TR == 'Private Transfer')
                                <a href="javascript:void(0)" class="fs--1 private_transfer_modal text-black"
                                    data-dataid="{{ $data }}"
                                    data-optionid="{{ $GPOD['product_option_id'] }}"><span
                                        class="fas fa-plus-circle"></span> </a>
                            @endif
                        </td>
                        <td>
                            <input class="form-control numberonly {{ $errors->has('adult') ? 'is-invalid' : '' }}"
                                type="text" @<?php if ($TR == 'Private Transfer'): ?>readonly <?php endif ?>
                                name="adult[{{ $data }}][{{ $key }}]"
                        ...
                        <td>
                            <input
                                ...
                                @<?php if ($TR == 'Private Transfer'): ?>readonly <?php endif ?> type="text"
                                name="child_price[{{ $data }}][{{ $key }}]"
