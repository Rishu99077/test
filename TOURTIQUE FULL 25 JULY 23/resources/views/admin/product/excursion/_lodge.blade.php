...
<div class="row lodge_div">
    ...
    <input type="hidden" name="lodge_id[]" value="{{$GPL['id']}}">

   
    <div class="col-md-6 mb-3">
        <label class="form-label" for="title">{{ translate('Lodge Title') }} </label>
    ...
    <div class="col-md-6">    
        <label class="form-label" for="title">{{ translate('Lodge Description') }}  </label>
        <textarea
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input
                            ...
                            type="text" name="lodge_price_adult[{{ $data }}]"
                            placeholder="{{ translate('Adult Price') }}" value="{{ $GPL['adult_price'] }}" />
                        @error('lodge_price_adult')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        @enderror
                    ...
                    <td>
                        ...
                        <input class="form-check-input mt-2 tour_child_allowed" id="flexCheckDefault" id="option_status"
                            type="checkbox" name="lodge_child_allowed[{{ $data }}]" value="1"
                            @php if($GPL['product_id'] != "")
                                {
                                    if($GPL['child_allowed'] == 1)
                                    {
                                        echo " checked";
                                    }
                                }else{
                    ...
                    <td>
                        ...
                        <input
                            ...
                            type="text" name="lodge_price_child[{{ $data }}]"
                            placeholder="{{ translate('Child Price') }}"
                            @php if($GPL['child_allowed'] != 1 && !isset($append) && $get_product['id'] != "")
                                    {
                                        echo "value=' N/A' readonly";
                                    }else{
                                        echo "value=".$GPL['child_price']."";
                                    } @endphp>
                        @error('lodge_price_child')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        @enderror
                    ...
                    <td>
                        ...
                        <input class="form-check-input mt-2 tour_infant_allowed" id="flexCheckDefault"
                            id="option_status" type="checkbox" name="lodge_infant_allowed[{{ $data }}]"
                            value="1"
                            @php if($GPL['product_id'] != "")
                            {
                                if($GPL['infant_allowed'] == 1)
                                {
                                    echo " checked";
                                }
                            }else{
                    ...
                    <td>
                        <input
                            ...
                            type="text" name="lodge_price_infant[{{ $data }}]"
                            placeholder="{{ translate('Infant Price') }}"
                            @php if($GPL['infant_allowed'] != 1 && !isset($append)&& $get_product['id'] != "")
                            {
                                echo "value=' N/A' readonly";
                            }else{
                                echo "value=".$GPL['infant_price']."";
                            } @endphp>
                        @error('lodge_price_infant')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                        @enderror
    ...
    <div class="col-md-12 content_title ">
        <h5 class="text-black mt-2 fw-semi-bold fs-0">{{ translate('Price Details') }}
            <small>({{ translate('Enter no of person allowed in rooms') }})</small>
        </h5>
        <table class="table table-responsive">
            <thead>
            ...
            <tbody id="lodge_price_tbody{{ $data }}">
                @foreach ($ProductLodgePrice as $PLP)
                    @include('admin.product.excursion._lodge_price')
                @endforeach
