...
if($GPO['product_id'] != "")
        ...
        <input type="hidden" name="tour_week_infant_allowed[{{ $data }}][]" value="{{ $Vl }}">

    </td>
    <td>
        <input class="form-control numberonly {{ $errors->has('tour_price_infant') ? 'is-invalid' : '' }}"
            type="text" name="tour_week_price_infant[{{ $data }}][]"
            placeholder="{{ translate('Infant Price') }}"
            @php if($GPOWT['infant_allowed'] != 1 && !isset($append)&& $get_product['id'] != "")
                    {
                        echo "value=' N/A' readonly";
                    }else{
                        echo "value=".$GPOWT['infant_price']."";
                    } @endphp>
