...
@section('content')
    ...
    <form class="row g-3 " method="POST" action="{{ route('admin.cities.add') }}" enctype="multipart/form-data">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body ">
                    <div class="row">
                        ...
                        <input id="" name="id" type="hidden" value="{{ $get_city['id'] }}" />
                        ...
                        <div class="col-md-4 content_title ">
                              ...
                              <select class="form-select single-select country  {{ $errors->has('country') ? 'is-invalid' : '' }}"
                                  ...
                                  @foreach ($country as $C)
                                      <option value="{{ $C['id'] }}" 
                                      {{ getSelected($C['id'], old('country', @$get_city['country'])) }}
                                      > {{ $C['name'] }}</option>
                                  @endforeach
                              </select>
                        ...
                        <div class="col-md-8">
                            <label class="form-label" for="name">{{ translate('City name') }}<span class="text-danger">*</span></label>
                            <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" placeholder="{{ translate('City name') }}" id="name" name="name" type="text" value="{{ $get_city['name'] }}" />
