...
@section('content')
    ...
    <div class="row mb-3">
        <form action="" method="post" class="mb-3" id="product_filter">
            ...
            <div class="row">
                <div class="col-lg-2">
                    <input type="text" name="All" class="form-control search-input" value="{{ $common['upcoming_order_all'] }}"
                        placeholder="{{ translate('Product Name') }}">
                </div>

                <div class="col-md-2">
                    <input class="form-control datetimepicker" id="timepicker3" name="Order_date" type="text" placeholder="Y-m-d to Y-m-d" value="{{ $common['upcoming_order_date'] }}" data-options='{"mode":"range","dateFormat":"Y-m-d","locale":"en"}' style="height: 38px;" />
                </div>

                <div class="col-lg-2">
                    <input type="text" name="OrderID" class="form-control search-input" value="{{ $common['upcoming_order_id'] }}"
                        placeholder="{{ translate('Order ID') }}">
                </div>

                <div class="col-lg-1">
                    <input type="text" name="Name" class="form-control search-input" value="{{ $common['upcoming_order_name'] }}"
                        placeholder="{{ translate('Name') }}">
                </div>

                <div class="col-lg-1">
                    <input type="text" name="Email" class="form-control search-input" value="{{ $common['upcoming_order_email'] }}"
                        placeholder="{{ translate('Email') }}">
                </div>

                <div class="col-lg-2">
                    <button class="btn btn-success" type="submit" name="filter" id="filter" value="filter" >{{ translate('Filter') }}</button>
                    <a href="{{ route('admin.orders') }}"><button class="btn btn-danger"
                            type="submit" id="reset" name="reset" value="reset" >{{ translate('Reset') }}</button></a>
                </div>

                <div class="col-lg-2">
                    <a href="{{ route('admin.upcoming_booking_report_export') }}"><button class="btn btn-primary float-end"
                            ...
                            @foreach ($optionArr as $key => $value)
                                <tr class="btn-reveal-trigger">
                                    ...
                                    <td class="order py-2 align-middle white-space-nowrap">
                                        <a
                                            href="{{ route('admin.orders.view', ['id' => encrypt($value['id'])]) }}"><strong>#{{ $value['order_id'] }}</strong></a>
                                        <br />
                                        <a class="text-body"
                                            href="{{ route('admin.orders.view', ['id' => encrypt($value['id'])]) }}">
                                            <strong>{{ $value['title'] }}</strong></a>
                                    </td>
