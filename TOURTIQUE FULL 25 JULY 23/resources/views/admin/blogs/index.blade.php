...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="col-auto ms-auto">
            <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.blogs.add') }}" type="button"><span
                ...
                <div class="card-body pt-0">
                    <div class="tab-content">
                        <div class="tab-pane preview-tab-pane active" role="tabpanel"
                                    ...
                                    <tbody>
                                        ...
                                        @if (!$get_blog->isEmpty())
                                            @foreach ($get_blog as $key => $value)
                                                <tr>
                                                    <td>{{ $key + $get_blog->firstItem() }}</td>
                                                    <td>
                                                        @php
                                                            $get_blog_language = App\Models\BlogLanguage::where('blog_id', $value['id'])->get();
                                                        @endphp
                                                        @if($get_blog_language)
                                                            {{  getLanguageTranslate($get_blog_language, $lang_id, $value['id'], 'title', 'blog_id')   }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-end">
                                                        <div>
                                                            <a class="btn p-0"
                                                                href="{{ route('admin.blogs.edit', encrypt($value['id'])) }}"
                                                            ...
                                                            <a class="btn p-0" onclick="return doconfirm();"
                                                                href="{{ route('admin.blogs.delete', encrypt($value['id'])) }}"
                                                                type="button" data-bs-toggle="tooltip"
                            ...
                            <div>
                                {{ $get_blog->appends(request()->query())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
