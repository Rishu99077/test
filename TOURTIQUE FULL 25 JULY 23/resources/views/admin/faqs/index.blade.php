...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="col-auto ms-auto">
            <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.faqs.add') }}" type="button"><span
                ...
                <div class="card-body pt-0">
                    <div class="tab-content">
                        <div class="tab-pane preview-tab-pane active" role="tabpanel"
                                    ...
                                    <tbody>
                                        ...
                                        @if (!empty($get_faqs))
                                            @foreach ($get_faqs as $key => $value)
                                                <tr>
                                                    <td>{{ $key + 1 }}</td>
                                                    <td>{{ getLanguageTranslate($get_faq_category_language, $lang_id, $value['id'], 'category_name', 'faq_category_id') }}</td>
                                                    <td>    
                                                        @php
                                                            $get_faqs_language = App\Models\FaqLanguage::where('faq_id', $value['id'])->get();
                                                        @endphp
                                                        @if($get_faqs_language)
                                                            {{  getLanguageTranslate($get_faqs_language, $lang_id, $value['id'], 'question', 'faq_id')   }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-end">
                                                        <div>
                                                            <a class="btn p-0"
                                                                href="{{ route('admin.faqs.edit', encrypt($value['id'])) }}"
                                                            ...
                                                            <a class="btn p-0" onclick="return doconfirm();"
                                                                href="{{ route('admin.faqs.delete', encrypt($value['id'])) }}"
                                                                type="button" data-bs-toggle="tooltip"
                            ...
                            <div>
                                {{ $get_faqs_details->appends(request()->query())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
