...
<div class="row faq_div">
    ...
    <div class="col-md-6 mb-3">
        ...
        <label class="form-label" for="title">{{ translate('Affiliate Faq Answer') }}
        </label>
        <textarea class="form-control  {{ $errors->has('answer.' . $lang_id) ? 'is-invalid' : '' }}"
            placeholder="{{ translate('Enter Affiliate Faq Answer') }}" id="title" name="answer[{{ $lang_id }}][]">
        {{ getLanguageTranslate($get_affiliate_page_faq_language, $lang_id, $FAQS['id'], 'answer', 'page_faq_id') }}
        </textarea>
