...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="col-auto ms-auto">
            <a class="btn btn-falcon-primary me-1 mb-1 mt-1" href="{{ route('admin.time_slot.add') }}" type="button"><span
                        ...
                        <div class="tab-pane preview-tab-pane active" role="tabpanel"
                            ...
                            <div class="table-responsive scrollbar">
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php $i = 1; ?>
                                        @if (!$get_option_time->isEmpty())
                                            @foreach ($get_option_time as $key => $value)
                                                <tr>
                                                    <td>{{ $key + $get_option_time->firstItem() }}</td>
                                                    <td>{{ $value['title'] }}</td>
                                                    <td>
                                                    ...
                                                    <td class="text-end">
                                                        <div>
                                                            <a class="btn p-0"
                                                                href="{{ route('admin.time_slot.edit', encrypt($value['id'])) }}"
                                                            ...
                                                            <a class="btn p-0" onclick="return doconfirm();"
                                                                href="{{ route('admin.time_slot.delete', encrypt($value['id'])) }}"
                                                                type="button" data-bs-toggle="tooltip"
                            ...
                            <div>
                                {{ $get_option_time->appends(request()->query())->links() }}
                            </div>
                        </div>
                    </div>
                </div>
