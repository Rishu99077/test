...
@section('content')
    ...
    <form class="row g-3 " method="POST" action="{{ route('admin.limousine_page.add') }}" enctype="multipart/form-data">
        @csrf
                        ...
                        <div class="col-md-6 content_title">
                            ...
                            <div class="input-group mb-3">
                                <input class="form-control " type="file" name="side_banner"
                                    aria-describedby="basic-addon2" onchange="loadFile(event,'limousin_side_banner')"
                                    id="side_banner" />
                                ...
                                <div class="avatar shadow-sm img-thumbnail position-relative blog-image-cls">
                                    <div class="h-100 w-100  overflow-hidden position-relative">
                                        <img src="{{ isset($MetaGlobalSideBanner['image']) && $MetaGlobalSideBanner['image'] != '' ? asset('uploads/side_banner/' . $MetaGlobalSideBanner['image']) : asset('uploads/placeholder/placeholder.png') }}"
                                            id="limousin_side_banner" width="100" alt="" />
                                    </div>
                                </div>
                            </div>
                        </div>
                         ...
                         <div class="colcss">
                            <h3 class="text-black mt-2">{{ translate('Limousine Banners') }}</h3>
                            @php
                                $data = 0;
                            @endphp
                            @foreach ($get_slider_images as $LSI)
                                @include('admin.limousine_page._slider_images')
                                @php
                                    $data++;
                                @endphp
                            @endforeach
        ...
        $(document).ready(function() {
            ...
            $('#add_slider').click(function(e) {
                var length = $(".over_view_row").length;
                    ...
                    var ParamArr = {
                        'view': 'admin.limousine_page._slider_images',
                        'data': count,
                    }
