...
class BreakdownController extends Controller
{
    ...
    public function activity_price_brakdown(Request $request)
    {
        ...
        if ($get_product_options && count($data) > 0) {
            ...
            $ProductOptionLanguage = ProductOptionLanguage::where('option_id', $get_product_options['id'])->where('language_id', $language)->first();
            //option
                ...
                if ($data['is_week_days_id'] == 0 && $data['is_period_pricing'] == 0 && $ProductOptionGroupPercentage == '') {
                    if ($request->date != '' && $req_child_qty == 0 && $req_infant_qty == 0 && $get_product_options['minimum_people'] == $req_adult_qty) {
                        ...
                        if ($ProductOptionWeekTour != '') {
                            $adult_price = $ProductOptionWeekTour->adult != '' ? get_partners_dis_price(ConvertCurrency($ProductOptionWeekTour->adult), $activity_id, $request->user_id, 'weekdays', 'excursion') : 0;
                            $child_price = $ProductOptionWeekTour->child_price != '' ? get_partners_dis_price(ConvertCurrency($ProductOptionWeekTour->child_price), $activity_id, $request->user_id, 'weekdays', 'excursion') : 0;
                            $infant_price = $ProductOptionWeekTour->infant_price != '' ?  get_partners_dis_price(ConvertCurrency($ProductOptionWeekTour->infant_price), $activity_id, $request->user_id, 'weekdays', 'excursion') : 0;
                        ...
                        if ($ProductOptiossnPeriodPricing != '') {
                            $adult_price = $ProductOptiossnPeriodPricing->adult_price != '' ? ConvertCurrency($ProductOptiossnPeriodPricing->adult_price) : 0;
                            $child_price = $ProductOptiossnPeriodPricing->child_price != '' ? ConvertCurrency($ProductOptiossnPeriodPricing->child_price) : 0;
                            $infant_price = $ProductOptiossnPeriodPricing->infant_price != '' ? ConvertCurrency($ProductOptiossnPeriodPricing->infant_price) : 0;
            ...
            if (isset($data['tour_transfer_id'])) {
                if ($data['tour_transfer_id'] > 0) {
                    ...
                    if ($get_option_details) {
                        ...
                        $get_tour_option_arr['adult_price'] = $get_option_details['edult'] > 0 ?  get_partners_dis_price(ConvertCurrency($get_option_details['edult']), $activity_id, $request->user_id, 'transfer_option', 'excursion') : 'N/A';
                        $get_tour_option_arr['child_price'] = $get_option_details['child_price'] > 0 ? get_partners_dis_price(ConvertCurrency($get_option_details['child_price']), $activity_id, $request->user_id, 'transfer_option', 'excursion') : 'N/A';
                        $get_tour_option_arr['infant_price'] = $get_option_details['infant'] > 0 ? get_partners_dis_price(ConvertCurrency($get_option_details['infant']), $activity_id, $request->user_id, 'transfer_option', 'excursion') : 'N/A';
            ...
            if (isset($data['data']['tour_adult_price']['tour_upgrade_arr'])) {
                ...
                foreach ($tour_upgrade_arr as $upgrade_key => $tour_upgrade_value) {
                    ...
                    if ($get_product_option_tour_upgrade) {
                        ...
                        $req_child_qty = 0;
                        $req_infant_qty = 0;
                        $adult_price = get_partners_dis_price(ConvertCurrency($get_product_option_tour_upgrade['adult_price']), $activity_id, $request->user_id, 'tour_upgrade_option', 'excursion') ?? 0;
                        $child_price =  get_partners_dis_price(ConvertCurrency($get_product_option_tour_upgrade['child_price']), $activity_id, $request->user_id, 'tour_upgrade_option', 'excursion') ?? 0;
                        $infant_price = get_partners_dis_price(ConvertCurrency($get_product_option_tour_upgrade['infant_price']), $activity_id, $request->user_id, 'tour_upgrade_option', 'excursion') ?? 0;
        ...
        $resoponse['tax_amount'] = $tax_amount == 0  ? 0 : number_format($tax_amount, 2);
        $resoponse['sub_total'] = number_format($total_amount, 2);
        if ($total_amount != 0) {
            $resoponse['service_charge'] = $service_charge;
            $resoponse['tax'] = $tax > 0  ? number_format($tax, 2) : 0;
    ...
    public function get_tour_option_time(Request $request)
    {
        ...
        if ($get_product_options['is_private_tour'] == 1) {
            ...
            $total_allowed                 = $ProductOptionPrivateTourPrice['total_allowed'];
            $private_per_price             = $adult_price;
            $private_tour_total_passanger_ = 0;
            $total_private_per_price       = 0;
            $private_tour_pasenger         = $request->qty;
            if ($total_allowed > 0) {
                if ($private_tour_pasenger > $total_allowed) {
                    $private_tour_total_passanger_ = ceil($private_tour_pasenger / $total_allowed);
                } else {
            ...
            $get_option['total'] = $total_private_per_price;
        }


        $get_option['is_private_tour'] = $get_product_options['is_private_tour'];
        ...
        if ($get_option_details) {
            if ($get_option_details->is_input == 1) {
                $tour_transfer_adult_price =  get_partners_dis_price(ConvertCurrency($get_option_details->edult), $activity_id, $request->user_id, 'transfer_option', 'excursion');
                $tour_transfer_child_price = get_partners_dis_price(ConvertCurrency($get_option_details->child_price), $activity_id, $request->user_id, 'transfer_option', 'excursion');
                $tour_transfer_infant_price = get_partners_dis_price(ConvertCurrency($get_option_details->infant), $activity_id, $request->user_id, 'transfer_option', 'excursion');
            }
        }

        $get_option['tour_transfer_adult_price'] = $tour_transfer_adult_price >= 0 &&  $tour_transfer_adult_price != "" ? $tour_transfer_adult_price  : 0;
        $get_option['tour_transfer_child_price'] = $tour_transfer_child_price  >= 0 &&  $tour_transfer_child_price != "" ? $tour_transfer_child_price  : 0;
        $get_option['tour_transfer_infant_price'] = $tour_transfer_infant_price >= 0  &&  $tour_transfer_infant_price != "" ? $tour_transfer_infant_price  : 0;
    ...
    public function activity_group_rates_breakdown(Request $request)
    {
        ...
        if ($qty > 0) {
            ...
            if ($ProductGroupPercentage) {
                ...
                $BreakDown['service_charge_allowed']         = $ProductGroupPercentage['service_charge_allowed'];
                $BreakDown['tax_allowed']                    = $ProductGroupPercentage['tax_allowed'];
                $BreakDown['tax_percentage']                 = $ProductGroupPercentage['tax_percentage'];
                $BreakDown['tax_amount']                     = $tax_amount;
                $BreakDown['service_charge']                 = $service_charge;
                ...
                $BreakDown['qty']          = $qty;
                $BreakDown['total']        = number_format($amount, 2);
                $BreakDown['total_amount'] = number_format($total_amount, 2);
