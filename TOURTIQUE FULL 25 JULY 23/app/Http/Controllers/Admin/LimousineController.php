...
class LimousineController extends Controller
{
    public function index(Request $request)
    {
        ...
        Session::put('SubMenu', 'Limousine');
        Session::put('SubSubMenu', 'Add Limousine');

        $addDaysDate = Carbon::now()->addDays(14);
        $expired_Product =  Product::select('products.*', 'product_language.description as title')->join('product_language', 'products.id', '=', 'product_language.product_id')->where('products.rate_valid_until', "<=", $addDaysDate)->where('products.rate_valid_until', ">=", Carbon::now())->where('products.product_type', 'limousine')->groupBy('products.id')->get();

        $get_product = Product::select('products.*', 'product_language.description as title')
            ->where('products.is_delete', 0)
            ->where('products.product_type', 'limousine')
        ...
        if ($request->ajax()) {
            ...
            return view('admin.product.limousine._listing', compact('common', 'get_product','lang_id'))->render();
        } else {
            $get_product = $get_product->paginate(config('adminconfig.records_per_page'));
            return view('admin.product.limousine.index', compact('common', 'get_product', 'country', 'expired_Product','lang_id'));
        }
    }

    //Add Yacht
    public function addlimousine(Request $request, $id = '')
    {
        Session::put('TopMenu', 'Product');
        Session::put('SubMenu', 'Limousine');
        Session::put('SubSubMenu', 'Add Limousine');
        ...
        $common            = [];
        $common['title']   = translate('Add Limousine');
        ...
        $get_product_deafault_title            = DefaultToolTipTitle::where('type','limousine')->get()->toArray();
        $get_product_setting                   = '';
        $get_limousine_locations               = [];
        $get_limousine_types                   = [];
        $get_limousine_locations               = BoatLocations::where('type', 'limousine')->where(['status' => 'Active'])->get();
        $get_limousine_types                   = BoatTypes::where('type', 'limousine')->where(['status' => 'Active'])->get();
        ...
        if ($request->isMethod('post')) {
            ...
            // NEW ------------------- 
            // $req_fields['transfer_option_price.*'] = 'required';
            // $req_fields['transfer_option_quantity.*'] = 'required';

            // $req_fields['pro_popup_link'] = 'required';
            ...
            $errormsg = [
                ...
                // NEW ------------------- 
                'transfer_option_price.*' => translate('Transfer option price'),
                'transfer_option_quantity.*' => translate('Transfer option quantity'),

                'pro_popup_link' => translate('Product popup link'),
            ...
            if ($request->id != '') {
                ...
                // PRODUCT VOUCHER
                $get_ProductVoucher = ProductVoucher::where(['product_id' => $request->id, 'type' => 'limousine'])->get();
                ...
                // Productyachttransferoption
                $get_Product_transfer = Productyachttransferoption::where('product_id', $request->id)->get();
                foreach ($get_Product_transfer as $key => $val_tra) {
                    if (!in_array($val_tra['id'], $request->transfer_id)) {
                        Productyachttransferoption::where('id', $val_tra['id'])->delete();
                    }
                }
            ...
            $Product['excursion_type']                 = $request->limousine_type;
            ...
            $Product['boat_type']                      = $request->boat_type != "" ? implode(',', $request->boat_type) : "";
            ...
            $Product['product_type']              = 'limousine';
            ...
            if (!empty($get_languages)) {
                foreach ($get_languages as $key => $value) {
                    ...
                    if ($request->description_title) {
                        $MetaGlobalLanguageDescription =  MetaGlobalLanguage::where('meta_title','limousine_description_title')->where('meta_parent','limousine_description')->where('product_id',$product_id)->where('language_id',$value['id'])->first();
                        if(!$MetaGlobalLanguageDescription){
                            $MetaGlobalLanguageDescription  = new MetaGlobalLanguage();
                        }
                            // print_die($request->description_title);
                            $MetaGlobalLanguageDescription->meta_parent = 'limousine_description';
                            $MetaGlobalLanguageDescription->meta_title  = 'limousine_description_title';
            ...
            // Product Setting
            ProductSetting::where(['product_id' => $product_id, 'type' => 'limousine'])->delete();
            ...
            if (!empty($request->fetaure_title)) {
                MetaGlobalLanguage::where('meta_title', 'limousine_feature_highlight')->where('language_id',$lang_id)->where('product_id', $product_id)->delete(); 
                foreach ($get_languages as $lang_key => $value) {
                    if (!getDataFromDB('meta_global_language',['language_id'=>$value['id'],'meta_title'=>'limousine_feature_highlight','product_id' => $product_id],'row')) {       
                        $MetaGlobalLanguage = new MetaGlobalLanguage();
                    
                        $MetaGlobalLanguage->meta_title  = 'limousine_feature_highlight';
            ...
            if (!empty($request->feature_highlight_title)) {
                ...
                foreach ($request->feature_highlight_id as $key => $FHT) {
                    ...
                    $YachtFeatureHighlight['type']       = "limousine";
            ...
            //Boat Specification
            if (!empty($request->limousine_boat_specification_title)) {
                MetaGlobalLanguage::where('meta_title', 'limousine_boat_specification')->where('language_id',$lang_id)->where('product_id', $product_id)->delete();

                foreach ($get_languages as $lang_key => $value) {
                    if (!getDataFromDB('meta_global_language',['language_id'=>$value['id'],'meta_title'=>'limousine_boat_specification','product_id' => $product_id],'row')) {       
                        $MetaGlobalLanguage = new MetaGlobalLanguage();
                        $MetaGlobalLanguage->meta_title  = 'limousine_boat_specification';
                        $MetaGlobalLanguage->title       = isset($request->limousine_boat_specification_title[$value['id']]) ?  $request->limousine_boat_specification_title[$value['id']] : $request->limousine_boat_specification_title[$lang_id];
                        $MetaGlobalLanguage->content     = isset($request->limousine_boat_specification_description[$value['id']]) ?  $request->limousine_boat_specification_description[$value['id']] : $request->limousine_boat_specification_description[$lang_id];
                        $MetaGlobalLanguage->language_id = $value['id'];
                        $MetaGlobalLanguage->product_id  = $product_id;
                        $MetaGlobalLanguage->status      = 'Active';
                        $MetaGlobalLanguage->save();
            ...
            if (!empty($request->boat_specification_key_title)) {
                ...
                foreach ($request->boat_specification_id as $key => $BI) {
                    ...
                    $YachtBoatSpecification['type']       = "limousine";
                            ...
                            foreach ($request->tour_week as $weekKey => $TW) {
                                if ($weekKey == $key) {
                                    ...
                                    foreach ($ProductOptionWeekTour_ as $WeekTour_key => $get_productoptionweektour_delete) {
                                        if (!in_array($get_productoptionweektour_delete['id'], $request->week_tour_id[$weekKey])) {
            ...
            // {{ Eigth Customer Group Tab Start }}
            if (isset($request->product_customer_group_id) && count($request->product_customer_group_id) > 0) {
                ...
                foreach ($request->product_customer_group_id as $CGKey => $PCGI) {
                    ...
                    //     $ProuductCustomerGroupDiscount = ProuductCustomerGroupDiscount::where(['id'=>$request->product_customer_group_discount_id[$CGKey],'type'=>"limousine"])->first();
                    // } else {
                    // }
                    ...
                    $ProuductCustomerGroupDiscount['type']              = "limousine";
            ...
            // {{ Eigth Customer Group Tab End }}


            // Voucher
            if (isset($request->voucher_title)) {
                if (count($request->voucher_title) > 0) {
                    if ($request->voucher_title) {
                        // ProductVoucher::where(['product_id' => $product_id])->delete();
                        ProductVoucherLanguage::where(['product_id' => $product_id, 'type' => "limousine"])->where('language_id',$lang_id)->delete();

                        foreach ($request->voucher_id as $key => $value_2) {
                            if ($value_2 != '') {
                                $product_voucher = ProductVoucher::where(['id' => $value_2, 'type' => 'limousine'])->first();
                            } else {
                            ...
                            $product_voucher['type'] = "limousine";
                            ...
                            foreach ($get_languages as $lang_key => $value) {
                                if (!getDataFromDB('product_voucher_language',['language_id'=>$value['id'],'product_id' => $request->id,'voucher_id'=> $product_voucher->id ],'row')) {   
                                    ...
                                    $ProductVoucherLanguage['type']        = "limousine";
            ...
            // {{ Ninth ToolTip Tab End }}


            // {{ tenth Amenties Tab Start }}
            // yatch_amenities_heading
            if (!empty($request->yatch_amenities_heading)) {
                MetaGlobalLanguage::where('meta_title', 'limousine_amenities')->where('product_id', $product_id)->delete();

                foreach ($get_languages as $lang_key => $value) {
                    if (!getDataFromDB('meta_global_language',['language_id'=>$value['id'],'meta_title'=>'limousine_amenities','product_id' => $product_id],'row')) {    
                        $MetaGlobalLanguage = new MetaGlobalLanguage();
                    
                        $MetaGlobalLanguage->meta_title  = 'limousine_amenities';
            ...
            // Product boats
            Productyachtrelatedboats::where(['product_id' => $product_id, 'type' => 'Limousine'])->delete();
            if (isset($request->boat) && count($request->boat) > 0) {
                if ($request->boat) {
                    foreach ($request->boat as $key => $value_boats) {
                        ...
                        $Productyachtrelatedboats['type']       = 'Limousine';
            ...
            $get_request_pop = ProductRequestPopup::where(['product_id' => $request->id ,'type'=>'limousine'])->get();
            foreach ($get_request_pop as $key => $pop_delete) {
            ...
            if (isset($request->request_description) && count($request->request_description) > 0) {
                if ($request->request_description) {
                    // ProductRequestPopup::where(['product_id' => $product_id ,'type'=>'limousine'])->delete();
                    ProductRequestPopupLanguage::where(['product_id' => $product_id ,'type'=>'limousine'])->where('language_id',$lang_id)->delete();
                    ...
                    foreach ($request->popup_id as $key => $value_7) {
                        ...
                        $ProductRequestPopup['type']       = 'limousine';
                        ...
                        foreach ($get_languages as $lang_key => $value) {
                            if (!getDataFromDB('product_request_popup_language',['language_id'=>$value['id'],'product_id' => $request->id,'request_popup_id'=> $ProductRequestPopup->id ],'row')) {       
                                ...
                                $ProductRequestPopupLanguage['type']        = 'limousine';
           ...
           // Expireance Icon middle heading End
            ...
            // EXPERIENECE ICON
            $get_ExpIcon = ProductExprienceIcon::where(['product_id'=> $product_id,'type'=>'limousine'])->where('position_type','middel')->get();
            ...
            if (!empty($request->exper_id)) {
                if ($request->exper_id) {
                    ...
                    ProductExprienceIconLanguage::where(['product_id'=> $product_id,'type'=>'limousine'])->where('language_id',$lang_id)->where('position_type','middel')->delete();
                    foreach ($request->exper_id as $key => $over_value) {                
                        ...
                        $ProductExprienceIcon->type        = "limousine";
                        ...
                        foreach ($get_languages as $lang_key => $value) {
                            if (!getDataFromDB('product_experience_icon_language',['language_id'=>$value['id'],'product_id' => $request->id,'experience_icon_id'=> $ProductExprienceIcon->id ,'position_type'=>'middel'],'row')) {   
                                ...
                                $ProductExprienceIconLanguage['type']               = 'limousine';
            ...
            // Expireance Icon Header
                ...
                // Expireance Icon Header heading start
                    if (!empty($request->header_experience_heading)) {
                        MetaGlobalLanguage::where('meta_parent', 'header_experience_heading')->where('language_id',$lang_id)->where('product_id',$product_id)->delete();
                ...
                if (!empty($request->header_exper_id)) {
                    if ($request->header_exper_id) {
                        ...
                        ProductExprienceIcon::whereNotIn('id', $header_exper_id_arr)->where('product_id',$product_id)->where('type','limousine')->where('position_type','upper')->delete();
                        ProductExprienceIconLanguage::where(['product_id'=> $product_id,'type'=>'limousine'])->where('position_type','upper')->where('language_id',$lang_id)->delete();
                        foreach ($request->header_exper_id as $value_key => $header_exper_value) {                
                            ...
                            $ProductExprienceIconHeader->type          = "limousine";
                            ...
                            foreach ($get_languages as $lang_key => $value) {
                                if (!getDataFromDB('product_experience_icon_language',['language_id'=>$value['id'],'product_id' => $request->id,'experience_icon_id'=> $ProductExprienceIconHeader->id ,'position_type'=>'upper'],'row')) {    
                                    ...
                                    $ProductExprienceIconLanguageHeader['type']               = 'limousine';
            ...
            if (!empty($request->opening_time_heading)) {
                    MetaGlobalLanguage::where('meta_parent', 'limousine_opening_heading')->where('language_id',$lang_id)->where('product_id',$product_id)->delete();

                    foreach ($get_languages as $key => $value) {
                        if (!getDataFromDB('meta_global_language',['language_id'=>$value['id'],'meta_parent'=>'limousine_opening_heading','product_id' => $product_id],'row')) {     

                            $MetaGlobalLanguage = new MetaGlobalLanguage();
                       
                            $MetaGlobalLanguage->meta_parent = 'limousine_opening_heading';
                            $MetaGlobalLanguage->meta_title  = 'heading_title';
                ...
                if (!empty($request->additional_heading)) {
                    MetaGlobalLanguage::where('meta_parent', 'limousine_additional_heading')->where('language_id',$lang_id)->where('product_id',$product_id)->delete();

                    foreach ($get_languages as $key => $value) {
                        if (!getDataFromDB('meta_global_language',['language_id'=>$value['id'],'meta_parent'=>'limousine_additional_heading','product_id' => $product_id],'row')) {     
                            $MetaGlobalLanguage = new MetaGlobalLanguage();
                        
                            $MetaGlobalLanguage->meta_parent = 'limousine_additional_heading';
                            $MetaGlobalLanguage->meta_title  = 'heading_title';
                ...
                if (!empty($request->faq_heading)) {
                    MetaGlobalLanguage::where('meta_parent', 'limousine_faq_heading')->where('language_id',$lang_id)->where('product_id',$product_id)->delete();
                  
                    foreach ($get_languages as $key => $value) {
                        if (!getDataFromDB('meta_global_language',['language_id'=>$value['id'],'meta_parent'=>'limousine_faq_heading','product_id' => $product_id],'row')) {     
                            $MetaGlobalLanguage = new MetaGlobalLanguage();
                        
                            $MetaGlobalLanguage->meta_parent = 'limousine_faq_heading';
                            $MetaGlobalLanguage->meta_title  = 'heading_title';
            ...
            return redirect()->route('admin.limousine')->withErrors([$status => $message]);
        }
        ...
        if ($id != '') {
            ...
            $id = checkDecrypt($id);
            $common['title'] = translate('Edit Limousine');
            ...
            $get_product_tooltip                   = ProductToolTip::where(['product_id' => $id, 'type' => "limousine"])->get();
            $get_product_option                    = ProductOptions::with('get_product_option_details')
                ->where('product_id', $id)
                ->get();
            $get_product_faq                      = ProductFaqs::where('product_id', $id)->get();
            $get_product_voucher                  = ProductVoucher::where(['product_id' => $id, 'type' => 'limousine'])->get();
            ...
            $get_product_tooltip_language         = ProductToolTipLanguage::where(['product_id' => $id, 'type' => "limousine"])->get();
            ...
            $get_product_voucher_language         = ProductVoucherLanguage::where(['product_id' => $id, 'type' => 'Limousine'])->get();
            ...
            $get_yatch_fetaure_highlight_heading  = MetaGlobalLanguage::where(['product_id' => $id, 'meta_title' => 'limousine_feature_highlight'])->get();
            $get_yatch_amenities_heading          = MetaGlobalLanguage::where(['product_id' => $id, 'meta_title' => 'limousine_amenities'])->get();
            $get_yatch_boat_specification_heading = MetaGlobalLanguage::where(['product_id' => $id, 'meta_title' => 'limousine_boat_specification'])->get();
            ...
            $get_product_deafault_title           = DefaultToolTipTitle::where('type', 'limousine')->get()->toArray();
            $get_product_related_boats            = Productyachtrelatedboats::where(['product_id' => $id, 'type' => 'Limousine'])->get();
            $meta_global_language_description =  MetaGlobalLanguage::where('meta_title','limousine_description_title')->where('meta_parent','limousine_description')->where('product_id',$id)->get();

            $opening_time_heading               = MetaGlobalLanguage::where('meta_parent', 'limousine_opening_heading')->where('product_id',$id)->get();
            $additional_heading                 = MetaGlobalLanguage::where('meta_parent', 'limousine_additional_heading')->where('product_id',$id)->get();
            $faq_heading                        = MetaGlobalLanguage::where('meta_parent', 'limousine_faq_heading')->where('product_id',$id)->get();

            

            $get_product_experience_icon           = ProductExprienceIcon::where(['product_id'=> $id,'type' => 'limousine','status'=>'Active'])->where('position_type','middel')->get();
            $get_product_experience_icon_language  = ProductExprienceIconLanguage::where(['product_id'=> $id,'type' => 'limousine'])->where('position_type','middel')->get();
            ...
            //Ecpireance Icon HEader
                $get_product_experience_icon_upper          = ProductExprienceIcon::where(['product_id'=> $id,'type' => 'limousine','status'=>'Active'])->where('position_type','upper')->get();
                $get_product_experience_icon_language_upper = ProductExprienceIconLanguage::where(['product_id'=> $id,'type' => 'limousine'])->where('position_type','upper')->get();
            ...
            $get_yatch_transfer_option = Productyachttransferoption::where('product_id', $id)->get();
        ...
        return view(
            'admin.product.limousine.addProduct',
            compact(
                'common',
                'meta_global_language_description',
                'get_yatch_fetaure_highlight_heading',
    ...
    public function delete($id)
    {
        ...
        if ($get_product) {
            ...
            ProductVoucherLanguage::where(['product_id' => $id, 'type' => "limousine"])->delete();
            ...
            ProuductCustomerGroupDiscount::where(['product_id' => $id, 'type' => "limousine"])->delete();
            ...
            ProductToolTip::where(['product_id' => $id, 'type' => "limousine"])->delete();
            ProductToolTipLanguage::where(['product_id' => $id, 'type' => "limousine"])->delete();
    ...
    public function duplicate($id)
    {
        ...
        if ($get_product) {
            ...
            $ProductSetting = ProductSetting::where(['product_id' => $id, 'type' => 'limousine'])->get();
            ...
            // Product ProductAddtionalInfo 
            $ProductVoucher = ProductVoucher::where(['product_id' => $id, 'type' => "limousine"])->get();
            $ProductVoucher->each(function ($item, $key) use ($NewId, $id) {
                $Voucher             = $item->replicate();
                $Voucher->product_id = $NewId;
                $Voucher->save();
                $ProductVoucherLanguage = ProductVoucherLanguage::where(['product_id' => $id, 'voucher_id' => $item->id, 'type' => "limousine"])->get();
            ...
            $ProuductCustomerGroupDiscount = ProuductCustomerGroupDiscount::where(['product_id' => $id, 'type' => 'limousine'])->get();
            ...
            $ProductToolTip = ProductToolTip::where(['product_id' => $id, 'type' => "limousine"])->get();
            $ProductToolTip->each(function ($item, $key) use ($NewId, $id) {
                $ToolTip             = $item->replicate();
                $ToolTip->product_id = $NewId;
                $ToolTip->save();

                $ProductToolTipLanguage = ProductToolTipLanguage::where(['product_id' => $id, 'tooltip_id' => $item->id, 'type' => "limousine"])->get();
            ...
            // Feature light 
            $MetaGlobalLanguage = MetaGlobalLanguage::where('product_id', $id)->where(['meta_title' => 'limousine_feature_highlight'])->get();
            ...
            // Feature Amenities
            $MetaGlobalLanguage = MetaGlobalLanguage::where('product_id', $id)->where(['meta_title' => 'limousine_amenities'])->get();
            $MetaGlobalLanguage->each(function ($item, $key) use ($NewId, $id) {
            ...
            $MetaGlobalLanguage = MetaGlobalLanguage::where('product_id', $id)->where(['meta_title' => 'limousine_boat_specification'])->get();
            $MetaGlobalLanguage->each(function ($item, $key) use ($NewId, $id) {
            ...
            $Productyachtrelatedboats = Productyachtrelatedboats::where(['product_id' => $id, 'type' => 'Limousine'])->get();
            $Productyachtrelatedboats->each(function ($item, $key) use ($NewId) {
             ...
             // Product ProductExprienceIcon  Middel
                $ProductExprienceIcon = ProductExprienceIcon::where(['product_id' => $id,'type' => 'limousine'])->where('position_type','middel')->get();
            ...
            // Product ProductExprienceIcon Header
                $ProductExprienceIcon = ProductExprienceIcon::where(['product_id' => $id,'type' => 'limousine'])->where('position_type','upper')->get();
                $ProductExprienceIcon->each(function ($item, $key) use ($NewId, $id) {
                    $ExpIcon             = $item->replicate();
                    $ExpIcon->product_id = $NewId;
                    $ExpIcon->save();
    ...
    public function limousine_types()
    {
        $common              = array();
        $common['title']     = "Limousine Types";
        Session::put("TopMenu", "Product");
        Session::put('SubMenu', 'Limousine');
        Session::put("SubSubMenu", "Limousine Types");
        $get_limousine_types      = BoatTypes::where('type', 'limousine')->paginate(config('adminconfig.records_per_page'));
        return view('admin.limousin_car_type.index', compact('common', 'get_limousine_types'));
    }

    ///Add Countries
    public function add_limousine_types(Request $request, $id = "")
    {
        $common = array();

        Session::put("TopMenu", "Settings");
        Session::put("SubMenu", "Boat Types");

        $common['title']     = translate("Add Limousine Types");
        $common['button']    = translate("Save");
        $get_limousine_types      = getTableColumn('boat_types');

        if ($request->isMethod('post')) {

            $req_fields               = [];
            $req_fields['limousine_type']  = "required";
            $errormsg = [
                "limousine_type"      => translate("Limousine Types"),
            ];
            ...
            $BoatTypes['boat_type']   = $request->limousine_type;
            $BoatTypes['type']        = "limousine";
            $BoatTypes['status']      = isset($request->status) ? 'Active' : 'Deactive';
            $BoatTypes->save();
            return redirect()->route('admin.limousine_types')->withErrors([$status => $message]);
        }
        ...
        if ($id != "") {
            ...
            $get_limousine_types          = BoatTypes::where('type', 'limousine')->where('id', $id)->first();

            if (!$get_limousine_types) {
                return back()->withErrors(["error" => "Something went wrong"]);
            }
        }


        return view('admin.limousin_car_type.add_limousine_type', compact('common', 'get_limousine_types'));
    }

    //Delete Limousine Car type
    public function delete_limousine_types($id)
    {
        ...
        $get_boat_types =  BoatTypes::where('id', $id)->where('type', 'limousine')->first();
        if ($get_boat_types) {
    ...
    // Limosine Car Locations -----------------------------------------------------------------------

    public function limousine_locations()
    {
        $common          = array();
        $common['title'] = "Limosine Locations";
        Session::put("TopMenu", "Product");
        Session::put("SubMenu", "Limousine");
        Session::put("SubSubMenu", "Limousine Locations");
        $get_limosine_locations = BoatLocations::where('type', 'limousine')->paginate(config('adminconfig.records_per_page'));
        return view('admin.limosine_car_locations.index', compact('common', 'get_limosine_locations'));
    }

    ///Add Boat Locations
    public function add_limousine_locations(Request $request, $id = "")
    {
        $common = array();
        Session::put("TopMenu", "Product");
        Session::put("SubMenu", "Limousine");
        ...
        $common['title']     = translate("Add Limousine Location");
        $common['button']    = translate("Save");

        $get_limosine_locations       = getTableColumn('boat_locations');

        if ($request->isMethod('post')) {
            $req_fields = [];

            $req_fields['limousine_location']  = "required";
            ...
            $BoatLocations['boat_location']   = $request->limousine_location;
            $BoatLocations['type']            = "limousine";
            $BoatLocations['status']          = isset($request->status) ? 'Active' : 'Deactive';

            $BoatLocations->save();
            return redirect()->route('admin.limousine_locations')->withErrors([$status => $message]);
        }
        ...
        if ($id != "") {
            ...
            $common['button']     = "Update";
            $get_limosine_locations          = BoatLocations::where('type', 'limousine')->where('id', $id)->first();

            if (!$get_limosine_locations) {
                return back()->withErrors(["error" => "Something went wrong"]);
            }
        }
        return view('admin.limosine_car_locations.add_limousine_location', compact('common', 'get_limosine_locations'));
    }

    public function delete_limousine_locations($id)
    {
        ...
        $status      = 'error';
        $message     = 'Something went wrong!';
        $get_boat_locations =  BoatLocations::where('id', $id)->where('type', 'limousine')->first();
