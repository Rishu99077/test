...
@include('admin.Common.topbar')
      <div class="d-flex align-items-center justify-content-between announce_ban bg_black mb_40">
         <div class="">
            <h3 class="f24 mb-0 text-white">All Admin Vendors</h3>
         </div>
      </div>
               ...
               <table>
                  ...
                  <tbody>
                        ...
                        <?php if ($data['user_details']['user_role']=='1') {?>
                          <td>
                             <?php if ($value['status']==0) { ?>
                                <a onClick="return moveconfirm();" id="Move_master" data-id="{{$value['VendorID']}}" class="btn btn-success">Move to master</a>
                             <?php }elseif ($value['status']==1) { ?> 
                               <a onClick="return remove_confirm();" id="Remove_master" data-id="{{$value['VendorID']}}" class="btn btn-danger">Remove from master</a>
                             <?php } ?>
                          </td>
                        <?php } ?>
                     ...
                     <tr class="alert alert-dismissible fade show text-center" role="alert">
                        <td class="text-center" colspan="9">No Vendors</td>
                     </tr>
                     <?php } ?>	
                  </tbody>
               </table>
...
<script type="text/javascript">
   function moveconfirm(){
        job=confirm("Are you sure to move this Vendor into master?");
        if(job!=true){
            return false;
        }
    }
    ...
    function remove_confirm(){
        job=confirm("Are you sure to remove this Vendor from master?");
        if(job!=true){
            return false;
        }
    }
