...
@include('admin.Common.topbar')
      <div class="d-flex align-items-center justify-content-between announce_ban bg_black mb_40">
          <div class="">
            <h3 class="f24 mb-0 text-white">All Vendors added by <span>{{$data['user_details']['firstname']}} {{$data['user_details']['lastname']}}</span></h3>
          </div>
         <div class="">
            <?php if ($data['user_details']['user_role']=='1') {?>
            <a href="{{('admin_vendors')}}" class="rec_btn">
              <span class="mr_10"><i class="fa fa-plus"></i> All restaurant vendors</span>
            </a>
            <?php }elseif ($data['user_details']['user_role']!='1') { ?>
              <a href="{{('master_vendors')}}" class="rec_btn">
                <span class="mr_10"><i class="fa fa-plus"></i> Add from master</span>
              </a>
            <?php } ?>
            <a href="{{('add_vendor')}}" class="rec_btn">
            ...
            <div class="col-md-6">
              <div class="row">
                  <div class="add_inve_box col-md-4 ">
                    ...
                    <input type="text" name="Vendorname" class="form-control Search_items" placeholder="Search by product name" value="<?php if (@$Vendorname != ''){
                      echo $Vendorname ; } ?>">
                  </div>
              </div>
            </div>
            <div class="col-md-2 mt-4">
              <div class="row">
                  <div class="add_inve_box col-md-12">
                      ...
                      <a id="clear" href="{{('vendors')}}" class="btn btn-danger clear">Clear</a>
                  </div>
              </div>
            </div>
        </div>
             ...
             <table>
                ...
                <tbody>
                   ...
                   <tr class="alert alert-dismissible fade show" role="alert">
                      ...
                      <td>
                         <div class="d-flex align-items-center inve_ed_del justify-content-center">
                            <a href="{{('add_vendor')}}?VendorID={{$value['VendorID']}}">
                            ...
                            <a href="{{url('delete_vendor')}}?VendorID={{$value['VendorID']}}" onClick="return doconfirm();">
                   ...
                   <tr class="alert alert-dismissible fade show text-center" role="alert">
                      <td class="text-center" colspan="10">No Vendors</td>
                   </tr>
                   <?php } ?>	
                </tbody>
             </table>
      ...
      <div class="row">
        <div class="col-md-12">
          {{$get_vendors->appends(request()->input())->links()}}
        </div>
      </div> 
   </div>
</div>
