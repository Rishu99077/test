...
class Bootstrap
{
    ...
    private function createCassandraDb(Cassandra $storage)
    {
        ...
        $cf->insert("token_oauth_users:testuser", array('__data' =>json_encode(array("password" => "password", "email" => "testuser@test.com", "email_verified" => true))));
    ...
    public function runPdoSql(\PDO $pdo)
    {
        ...
        // set up scopes
        $sql = 'INSERT INTO token_oauth_scopes (scope) VALUES (?)';
        ...
        $sql = 'INSERT INTO token_oauth_scopes (scope, is_default) VALUES (?, ?)';
        foreach (array('defaultscope1', 'defaultscope2') as $defaultScope) {
            $pdo->prepare($sql)->execute(array($defaultScope, true));
        }

        // set up clients
        $sql = 'INSERT INTO token_oauth_clients (client_id, client_secret, scope, grant_types) VALUES (?, ?, ?, ?)';
        ...
        // set up misc
        $sql = 'INSERT INTO token_oauth_access_tokens (access_token, client_id, expires, user_id) VALUES (?, ?, ?, ?)';
        ...
        $sql = 'INSERT INTO token_oauth_authorization_codes (authorization_code, client_id, expires) VALUES (?, ?, ?)';
        $pdo->prepare($sql)->execute(array('testcode', 'Some Client', date('Y-m-d H:i:s', strtotime('+1 hour'))));

        $sql = 'INSERT INTO token_oauth_users (username, password, email, email_verified) VALUES (?, ?, ?, ?)';
        ...
        $sql = 'INSERT INTO token_oauth_jwt (client_id, subject, public_key) VALUES (?, ?, ?)';
        ...
        if (!$this->dynamodb) {
            ...
            if (class_exists('\Aws\DynamoDb\DynamoDbClient')) {
                if ($client = $this->getDynamoDbClient()) {
                    ...
                    $config = array(
                        'client_table' => $prefix.'token_oauth_clients',
                        'access_token_table' => $prefix.'token_oauth_access_tokens',
                        'refresh_token_table' => $prefix.'token_oauth_refresh_tokens',
                        'code_table' => $prefix.'token_oauth_authorization_codes',
                        'user_table' => $prefix.'token_oauth_users',
                        'jwt_table'  => $prefix.'token_oauth_jwt',
                        'scope_table'  => $prefix.'token_oauth_scopes',
    ...
    {
        $tablesList = explode(' ', 'token_oauth_access_tokens token_oauth_authorization_codes token_oauth_clients token_oauth_jwt oauth_public_keys token_oauth_refresh_tokens token_oauth_scopes token_oauth_users');
    ...
    private function createDynamoDb(\Aws\DynamoDb\DynamoDbClient $client, $prefix = null)
    {
        ...
        $client->createTable(array(
            'TableName' => $prefix.'token_oauth_access_tokens',
        ...
        $client->createTable(array(
            'TableName' => $prefix.'token_oauth_authorization_codes',
            'AttributeDefinitions' => array(
        ...
        $client->createTable(array(
            'TableName' => $prefix.'token_oauth_clients',
            'AttributeDefinitions' => array(
        ...
        $client->createTable(array(
            'TableName' => $prefix.'token_oauth_jwt',
            'AttributeDefinitions' => array(
                array('AttributeName' => 'client_id','AttributeType' => 'S'),
        ...
        $client->createTable(array(
            'TableName' => $prefix.'token_oauth_refresh_tokens',
            'AttributeDefinitions' => array(
        ...
        $client->createTable(array(
            'TableName' => $prefix.'token_oauth_scopes',
            'AttributeDefinitions' => array(
        ...
        $client->createTable(array(
            'TableName' => $prefix.'token_oauth_users',
