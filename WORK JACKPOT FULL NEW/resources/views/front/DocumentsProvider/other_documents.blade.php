        ...
        <div class="contact_tabs">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link " id="contact-form-tab"  href="{{Route('provider.documents_contracts')}}" aria-controls="contract-form" >
                ...
                <li class="nav-item" role="presentation">
                    <a class="nav-link active" id="other-documents-tab"  href="{{Route('provider.documents_other_documents')}}" aria-controls="send-sms" >
                        <span class="tab_icon_a">
            ...
            <div class="tab-content" id="myTabContent">
                ...
                <!-- Other Document -->
                <div class="tab-pane fade active show" id="other-documents" role="tabpanel" aria-labelledby="other-documents-tab">
                    <div class="row menu-header mb-2">
                        <div class="col-md-10">
                            <p class="menu-title">{{__("customer.text_other_doc")}}</p>
                        </div>
                        <div class="col-md-2 text-end">
                            <a href="{{Route('other_documents')}}" class="btn green_btn">{{__("customer.text_send")}}</a>
                        </div>
                    </div>
                    @if(!$OtherDocuments->isEmpty())
                    <div class="job_card_wrapper_contract">
                           @foreach($OtherDocuments as $key => $document)
                            <div class="job_card_contract">
                                <div class="jcard_top_contract align-items-center justify-content-between">
                                    <div class="jc_left_s align-items-center justify-content-between">
                                        <div class="jc_left_l_doc">
                                            <span><img src="{{ asset('assets/images/docs_blue.png') }}" alt=""></span>
                                        </div>
                                    </div>
                                ...
                                <div class="jcard_center_self">
                                     <?php $full_title = $document['title']; ?>   
                                     <h5 class="heading_title"><?php echo (strlen($full_title) > 15) ? substr($full_title, 0, 15) . '...' : $full_title; ?></h5>
                                     <br>
                                     <p>{{__("customer.text_contract_no")}} {{$document['contract_id']}}</p>
                                     <p class="text-pr"><img src="{{ asset('assets/images/j6.png') }}" alt=""> <?php echo date('d/m/Y',strtotime($document['date']))?></p>
                                   
                                     <div class="jc_inbox_contract">
                                        <span class="jc_inbox1"></span> 
                                        <a href="{{Route('seeker.contract_details')}}?id={{$document['id']}}" class="jc_inbox2 f_bold clr_prpl f12 btn add_btn">
                                        <span>{{__("customer.text_view_details")}} </span>
                                        </a>
                                        <?php 
                                          if($document['document'] !=""){
                                              $link = url('/Images/otherdocuments',$document['document']);
                                          }else{
