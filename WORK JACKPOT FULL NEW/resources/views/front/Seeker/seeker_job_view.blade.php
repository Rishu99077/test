...
<div class="main_right">
	...
	<div class="job_card_wrapper sjob_det mb_10">
	    ...
	    <div class="job_card job_details_sec mb_20">
	        ...
	        <div class="jc_footer px-0">
	            ...
	            <p class="clr_grey f_regular f_14 description_p" style="word-wrap: break-word;">{{$job_details['description']}} 
  ...
  <div class="jo_rec box_s mb_20">
      <p class="f16 clr_prpl f_black">{{__("customer.text_requirements")}}</p>
      <ul>
        <?php foreach ($job_details['requirements'] as $key => $value1) { ?> 
          <li>{{$value1['requirements']}}</li>
        <?php } ?>
      </ul>
  </div>

  <?php if ($job_details['contract']=='') { ?>
  <div class="box_s d-flex align-items-center justify-content-between mb_20">
      <p class="f18 clr_prpl f_black m-0">{{__("customer.text_apply_for_job")}}</p>
      <a href="#" class="" id="add_contract" data-id="{{$job_details['id']}}">{{__("customer.text_contract_now")}} <span> <img src="{{ asset('assets/images/bot_ar.png') }}" alt=""></span></a>
  </div>
  <?php }else{ ?>
  <div class="box_s d-flex align-items-center justify-content-between mb_20">
      <p class="f18 clr_prpl f_black m-0">{{$job_details['contract_status']}}</p>
  </div>	
  <?php } ?>
