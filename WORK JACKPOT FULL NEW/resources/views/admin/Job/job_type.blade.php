...
<div class="main_right">
    ...
    <div class="top_announce d-flex align-items-center justify-content-between mb_20">
        <div class="top_ann_left">
            <a href="{{Route('job_types')}}" class="site_btn f16 f_bold blue__btn post_ad_disabled">{{$common['heading_title']}}</a>
        </div>
        <div class="top_ann_right d-flex align-items-center ">
            <form class="topan_search w-100 justify-content-xl-end">
                <a href="{{Route('job_type_add')}}" class="btn add_btn">{{__("admin.text_add_new")}}</a>
            </form>
        </div>
    </div>
             ...
             <?php $cnt = 1;  
              ...
              foreach ($job_types as $key => $value) { ?>
                <tr>
                  ...
                  <td>
                    <a href="{{Route('job_type_add')}}?id={{$value['id']}}"><span class="fas fa-edit"></span></a>
                    <a href="{{Route('job_type_delete')}}?id={{$value['id']}}" onClick="return doconfirm()"><span class="fa fa-trash text-danger"></span></a>
                  </td>
                </tr>
             <?php $cnt++; } }else{ ?>
                <tr>
    ...
    <div class="my_pagination">
        {{$Job_Types->appends(request()->query())->links() }}
    </div> 
</div>
